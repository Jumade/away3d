...
class AC3DParser extends ParserBase
{
		...
		while (_charIndex < _stringLen && hasTime()) {
			...
			switch (_trunk[0]) {
				...
				case "texture":
					...
					addDependency(Std.string(_meshList.length - 1), new URLRequest(tUrl));
	...
	private function buildMeshGeometry(mesh:Mesh):Void
	{
		...
		//var j:uint;
		var dic:Map<String, Int> = new Map<String, Int>();
		var ref:String;
		...
		while (i < _uvs.length) {
			...
			if (dic.exists(ref))
				...
				dic[ref] = Std.int(vertices.length/3);
				indices.push(dic[ref]);
				vertices.push(v1.x);
				vertices.push(v1.y);
				vertices.push(v1.z);
